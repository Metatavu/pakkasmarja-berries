...
export default class ContactsServiceImpl extends ContactsService {
  ...
  triggerChangeNotification(oldUser: any, newUser: any) {
    ...
    if (changes.length) {
      ...
      const { notifications } = contactConfig || {};
      const { contactUpdates } = notifications || {};

      if (
        contactUpdates &&
        Array.isArray(contactUpdates) &&
        contactUpdates.length
      ) {
        contactUpdates.forEach(email =>
          mailer.send(sender, email, subject, contents)
        );
      }
    }
  }
