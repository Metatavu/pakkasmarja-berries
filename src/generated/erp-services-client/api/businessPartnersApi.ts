...
import { SapBusinessPartner } from '../model/sapBusinessPartner';
...
export enum BusinessPartnersApiApiKeys {
}

export class BusinessPartnersApi {
    ...
    public setApiKey(key: BusinessPartnersApiApiKeys, value: string) {
        (this.authentications as any)[BusinessPartnersApiApiKeys[key]].apiKey = value;
    }
    ...
    public async listBusinessPartners (updatedAfter?: Date, firstResult?: number, maxResults?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Array<SapBusinessPartner>;  }> {
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: Array<SapBusinessPartner>;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "Array<SapBusinessPartner>");
